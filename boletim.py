...
pICMSO=float((boletim1.iloc[2,1]))

pICMSH=float((boletim1.iloc[3,1]))
...
crescPorc=round((boletim1.iloc[17,1])/100,2)
...
print("  Ontem              Hoje  \n")
print("R$ ", round(pICMSO/1000,3), " bi      R$ ", round(pICMSH/1000,3), "bi\n")
...
print("\n",("*💰Previsão"),mes,("/25 x"), mes, ("/24 - ICMS PRINCIPAL*"), "\n")

if (crescPorc>0):
    print ("\nCrescimento nominal de ", negrito(str(crescPorc).replace('.',',')),"% (R$", formatar(crescDif),")")
if (crescPorc<0):
    print ("\nDecréscimo nominal de ", negrito(str(crescPorc).replace('.',',')),"% (R$", formatar(crescDif),")")
